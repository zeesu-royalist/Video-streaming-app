...
import { Home, Film, Tv, FileText, Plus, Menu, X, ArrowRight, Search } from "lucide-react";
...
export default function LandingNavbar({ session }: LandingNavbarProps) {
  ...
  const navItems = [
    ...
    { name: "Subjects", href: "/videos", icon: Tv }, // redirects to videos list
    { name: "Documents", href: "/documents", icon: FileText },
    { name: "My List", href: "/dashboard", icon: Plus },
  ];
  ...
  return (
    <motion.header
      ...
      className="fixed top-0 left-0 right-0 z-50 border-b border-neutral-900/60 bg-[#0F0F0F]/65 backdrop-blur-md"
    >
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.name}
                href={item.href}
                className={`relative flex items-center gap-2.5 px-4.5 py-2 rounded-full text-xs font-semibold tracking-wide transition-all duration-300 cursor-pointer ${isActive
